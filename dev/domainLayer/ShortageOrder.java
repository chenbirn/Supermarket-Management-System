...
public class ShortageOrder extends Order {
    protected ShortageOrder(Product product, Collection<Supplier> suppliers, int max_id) {
        this(product, findCheapestSupplier(product, suppliers), max_id);
    }

    protected ShortageOrder(Product product, Supplier cheapest, int max_id) {
    ...
    protected ShortageOrder(int order_id, LocalDate orderDate, LocalDate deliveryDate, String contactNum, OrderStatus status, List<OrderProduct> products, Supplier supplier) {
    ...
    private static Supplier findCheapestSupplier(Product product, Collection<Supplier> suppliers) {
        ...
        if (cheapest == null) {
            throw new IllegalArgumentException("There is no supplier for this product (product_id=" + product.getId() + ")");
        }
